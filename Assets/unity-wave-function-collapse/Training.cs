...
class Training : MonoBehaviour{
	...
	// Resources.Load wants a path relative to a Resources folder, which may be
	// nested anywhere under Assets, so strip up to the last "Resources/".
	public static string ResourcePath(string assetpath){
		string path = assetpath.Trim().Replace(".prefab", "");
		int i = path.LastIndexOf("Resources/");
		return i < 0 ? path : path.Substring(i + "Resources/".Length);
	}

	public string AssetPath(UnityEngine.Object o){
		#if UNITY_EDITOR
		return ResourcePath(AssetDatabase.GetAssetPath(o));
		#else
		return "";
		#endif
	}
